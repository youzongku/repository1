...
function init_sales_order_refunds_confirm_receipt(BbcGrid) {
    var grid = new BbcGrid();
    grid.initTable($('#sales_order_refunds_confirm_receipt_table'), getSetting_sales_order_refunds_confirm_receipt());
    grid.initTable($("#detail_table"),detailSetting_confirm_receipt());
    grid.initTable($("#purchasing_info_table"),purchasingInfoSetting_confirm_receipt());

    $("#orderStatus").change(function(){
        $("#sales_order_refunds_confirm_receipt_table").jqGrid('setGridParam', {page:1}).trigger("reloadGrid");
    });
...
function getSetting_sales_order_refunds_confirm_receipt() {
    var setting = {
        ...
        colModel: [{name: "id", index: "id", width: "12%", align: "center", sortable: false, hidden: true},
            ...
            }},
            {
                ...
                formatter: function (cellvalue, options, rowObject) {
                    return "<a style='display:block;' onclick='checkDetails_sales_refunds_confirm_receipt(" + rowObject.id + ")'>查看详情</a>";
                }
            }
        ],
        viewrecords: true,
        rowNum: 10,
        rowList: [10, 20, 30],
        pager: "#sales_order_refunds_confirm_receipt_pagination",//分页
        caption: "发货单退款-确认收货",//表名称
        pagerpos: "center",
        pgbuttons: true,
        autowidth: true,
        ...
        serializeGridData: function () {
            return JSON.stringify(getSearchParam_sales_order_refunds_confirm_receipt($(this).jqGrid('getGridParam', 'postData')));
        }
    };
    return setting;
}

function getSearchParam_sales_order_refunds_confirm_receipt(postData) {
    var params = {
        pageSize: $('#sales_order_refunds_confirm_receipt_table').getGridParam('rowNum'),
        pageCount: $('#sales_order_refunds_confirm_receipt_table').getGridParam('page'),
        status: 4,
        searchSpan: $("#searchInput").val(),
        orderStartDate: $("#seachTime0").val(),
        orderEndDate: $("#seachTime1").val(),
        isProductReturn: $("#orderStatus").val(),
        sord: postData.sord,
        sidx: postData.sidx
    };
    return params;
...
function getList_sales_order_refunds_confirm_receipt() {
    $("#sales_order_refunds_confirm_receipt_table").jqGrid('setGridParam', {page:1}).trigger("reloadGrid");
}

/*查看详情-详细内容 start*/
function detailSetting_confirm_receipt() {
    var setting = {
        height : "auto", // 表格宽度
        width : 1067,
...
/*查看详情-采购信息 start*/
function purchasingInfoSetting_confirm_receipt() {
    var setting = {
        height : "auto", // 表格宽度
        width : 1067,
...
function checkDetails_sales_refunds_confirm_receipt(id){
    $("#sales_order_box").hide();
    $("#checkDetails").show();

    $('#SH_ORDER_ID').val(id);

    $('#OPERATE_SALES_ORDER_REFUNDS_CONFRIM_RECEIPT_DIV').empty();
    $('#returnProduct').hide();
    $('#saleOrderFinish').hide();
            ...
            if(data.data.length > 0) {
                ...
                for(var i =0; i< ds.length;i++) {
                    if(ds[i].type == 1) {
                        ...
                        content += '</div>';
                        $('#OPERATE_SALES_ORDER_REFUNDS_CONFRIM_RECEIPT_DIV').append(content);
                    }
...
function toConfirm_refunds_confirm_receipt(){
    layer.open({
        type: 1,
        title: "去确认",
        ...
        btn1:function(index) {
            ...
            ajax_post("/sales/shAudit",JSON.stringify(params),"application/json",function (data) {
                if(data.result) {
                    layer.msg("退款成功",{icon:1,time:3000},function () {
                        layer.close(index);
                        $("#sales_order_box").show();
                        $("#checkDetails").hide();
                        $("#sales_order_refunds_confirm_receipt_table").jqGrid('setGridParam', {page:1}).trigger("reloadGrid");
