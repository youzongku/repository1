...
function init_sales_order_refunds_finance_audit(BbcGrid) {
    var grid = new BbcGrid();
    grid.initTable($('#sales_order_refunds_finance_audit_table'), getSetting_sales_order_refunds_finance_audit());
    grid.initTable($("#detail_table"),detailSetting_finance_audit());
    grid.initTable($("#purchasing_info_table"),purchasingInfoSetting_finance_audit());

    $(".searchButton").click(function(){
        $("#sales_order_refunds_finance_audit_table").jqGrid('setGridParam', {page:1}).trigger("reloadGrid");
    });
}

function getSetting_sales_order_refunds_finance_audit() {
    var setting = {
        ...
        colModel: [{name: "id", index: "id", width: "12%", align: "center", sortable: false, hidden: true},
            ...
            }},
            {
                ...
                formatter: function (cellvalue, options, rowObject) {
                    return "<a style='display:block;' onclick='checkDetails_sales_refunds_finance_audit(" + rowObject.id + ")'>查看详情</a>";
                }
            }
        ],
        viewrecords: true,
        rowNum: 10,
        rowList: [10, 20, 30],
        pager: "#sales_order_refunds_finance_audit_pagination",//分页
        caption: "发货单退款-财务确认",//表名称
        pagerpos: "center",
        pgbuttons: true,
        autowidth: true,
        ...
        serializeGridData: function () {
            return JSON.stringify(getSearchParam_sales_order_refunds_finance_audit($(this).jqGrid('getGridParam', 'postData')));
        }
    };
    return setting;
}

function getSearchParam_sales_order_refunds_finance_audit(postData) {
    var params = {
        pageSize: $('#sales_order_refunds_finance_audit_table').getGridParam('rowNum'),
        pageCount: $('#sales_order_refunds_finance_audit_table').getGridParam('page'),
        status: 2,
...
function getList_sale_order_finance_audit() {
    $("#sales_order_refunds_finance_audit_table").jqGrid('setGridParam', {page:1}).trigger("reloadGrid");
}


/*查看详情-详细内容 start*/
function detailSetting_finance_audit() {
    var setting = {
        height : "auto", // 表格宽度
        width : 1067,
...
/*查看详情-采购信息 start*/
function purchasingInfoSetting_finance_audit() {
    var setting = {
        height : "auto", // 表格宽度
        width : 1067,
...
function checkDetails_sales_refunds_finance_audit(id){
    $("#sales_order_box").hide();
    $("#checkDetails").show();

    $('#SH_ORDER_ID').val(id);
    $('#OPERATE_SALES_ORDER_REFUNDS_FINANCE_AUDIT_DIV').empty();
            ...
            if(data.data.length > 0) {
                ...
                for(var i =0; i< ds.length;i++) {
                    if(ds[i].type == 1) {
                        ...
                        content += '</div>';
                        $('#OPERATE_SALES_ORDER_REFUNDS_FINANCE_AUDIT_DIV').append(content);
                    }
...
function toConfirm_finance_audit(){
    layer.open({
        type: 1,
        title: "去确认",
