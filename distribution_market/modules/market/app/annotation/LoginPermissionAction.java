...
import dto.marketing.promotion.PermissionRes;
...
public class LoginPermissionAction extends Action<LoginPermissionAction> {
	...
	public Promise<Result> call(Context context) throws Throwable {
		...
		try {
			...
			// 前台登录判断
			String res = ordinaryUser(1,context);
	...
	private String ordinaryUser(Integer mark,Context context) {
		String ltc = context != null ? cookies(context) : "";
		Serializable serializable = sessionService.get(mark + ltc);
	...
	private String cookies(Context context) {
		String cookie = "";
		try {
			cookie = context.request().cookie("TT_LTC").value();
		} catch (Exception e) {
