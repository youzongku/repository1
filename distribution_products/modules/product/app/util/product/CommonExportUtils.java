...
public class CommonExportUtils {
	 ...
	 * @param mergeKeyRowsLists 根据字段需要行合并的列
	 * @return
	 */
	public static String getFile(List<Map> sqlResult, Map<Integer, String> sqlResultKeyMap,
			Map<Integer, String> excelRowKeyMap, String fileName, String excelTitle, Map<Integer, Integer> excelWidthMap, List<Integer> rowsMergeLists, String mergeKey, List<Integer> mergeKeyRowsLists) {
		...
		insertDatas(sheet, cellStyleMap, sqlResult,sqlResultKeyMap,excelRowKeyMap,rowsMergeLists,mergeKey,mergeKeyRowsLists);
	 ...
	 * @param mergeKeyRowsLists 更加字段需要行合并的列
	 */
	private static void insertDatas(HSSFSheet sheet, Map<String, HSSFCellStyle> cellStyleMap,
			List<Map> sqlResult, Map<Integer, String> sqlResultKeyMap, Map<Integer, String> excelRowKeyMap, List<Integer> rowsMergeLists, String mergeKey, List<Integer> mergeKeyRowsLists) {
		// 从第4行开始是具体数据了
		int rowNum = 3;
		for (int i = 0; i < sqlResult.size(); i++) {
			  Map map = sqlResult.get(i);
			  if(!rowsMergeLists.isEmpty() && i>0){//无需根据指定字段操作行合并
			  ...
			  if(mergeKey!=null && !mergeKeyRowsLists.isEmpty() && i>0){//根据指定字段值相同操作行合并
				  ...
				  if(cellValue1.equals(cellValue0)){//指定字段值相同，操作需要合并的行进行合并
					  for(int line: mergeKeyRowsLists){
