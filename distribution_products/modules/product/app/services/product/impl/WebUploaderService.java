...
public class WebUploaderService {
    ...
    public String md5Check(String key){
        List<ContractAttachment> contractAttachments = attachmentMapper.checkMd5(key);
        if (!CollectionUtils.isEmpty(contractAttachments)) {
            return contractAttachments.get(0).getFilePath();
        } else {
            return null;
        }
    }
     ...
     * @return
     */
    public String chunksMerge(String folder, String ext, int chunks, String md5, String path, String cno, String fileName){
            ...
            try{
                ...
                if(chunks == files.size()){
                    ...
                    for(File file : files){
                        ...
                        //删除分片
                        if(!file.delete()){
                            Logger.error("分片[" + folder + "=>" + file.getName() + "]删除失败");
                        }
                    }
                    outChannel.close();
                    outputStream.close();
