...
public class StringUtils {
	...
	public static Integer getIntegerParam(Map<String, String[]> node, String field,Integer d) {
		...
		if(values == null || values.length <= 0 || isBlankOrNull(values[0])) {
			return d;
		}
	...
	public static boolean checkParam(Map<String, String[]> asFormUrlEncoded, String field) {
		if (asFormUrlEncoded != null 
				&& asFormUrlEncoded.containsKey(field) 
				&& asFormUrlEncoded.get(field).length > 0) {
			return asFormUrlEncoded.get(field).length > 1 ? 
					true : isNotBlankOrNull(asFormUrlEncoded.get(field)[0]) ? true : false;
		}
		return false;
	}
	public static Integer getIntegerParam(JsonNode node, String field,Integer d) {
		JsonNode values = node.get(field);
		if(values == null || isBlankOrNull(values.asText())) {
			return d;
		}
		return  values.asInt();
	}
	public static String getStringParam(JsonNode node, String field,String d) {
		JsonNode values = node.get(field);
		return values != null ? values.asText() : d;
	}
	public static boolean checkParam(JsonNode node, String field) {
		JsonNode values = node.get(field);
		if (values == null || isBlankOrNull(values.asText())) {
			return false;
		}
