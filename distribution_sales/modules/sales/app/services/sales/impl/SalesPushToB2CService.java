...
//			Logger.info("推送至hk前，进行税金计算");    暂时注释计算税金代码
			...
			for (SalesToB2cIterm iterm : iterms) {
				try {
					...
					if(null != iterm.getOrderLevel() && 50 == iterm.getOrderLevel()) {
						...
						iterm.setBusinessRemark(iterm.getCremark());
					}
					if (shop != null) {
					...
					boolean ctranction = StringUtils.isNotEmpty(iterm.getCtransactionid())
							&& iterm.getCtransactionid().startsWith("TT_BBC_");
					// add by xuse 内部分销商同步外部订单交易号 外部分销商如果是系统自动支付则同步采购交易
					if (iterm.getDistributorType() != null && iterm.getDistributorType() == 3) {
						iterm.setCtransactionid(ctranction ? iterm.getTradeNo() : iterm.getCtransactionid());
					} else {
						String transactionId = ctranction && iterm.getPurchasePayno() != null ? iterm.getPurchasePayno()
								: iterm.getCtransactionid();
						String paymentType = ctranction && iterm.getPurchasePayno() != null ? iterm.getPurchasePaytype()
								: iterm.getCpaymentid();
					...
					//快递单号 和 电子面单内容 2017-5-18
					KdnOrder kdnOrder = kdnOrderMapper.selectBySalesOrderNo(iterm.getCordernumber());
					if (kdnOrder != null) {
						iterm.setLogisticCode(kdnOrder.getLogisticCode());
						iterm.setPrintTemplate(kdnOrder.getPrintTemplate());
					}
