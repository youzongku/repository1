...
public class CookieTrackingFilter implements IFilter {
	...
	public Promise<Result> call(Context context, FilterExecutionChain chain)
		...
		if (ltc != null) {
			// set ltc cookie HttpOnly true
			context.response().setCookie(TT_LTC, ltc, 10 * 365 * 24 * 60 * 60, null, null, false, true);
			// context.response().setCookie("name-in-filter", "value-in-filter", 1000000, null, null, false, true);
		}
		if (stc != null) {
