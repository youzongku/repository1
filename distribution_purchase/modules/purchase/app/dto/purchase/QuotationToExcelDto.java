 ...
 */
public class QuotationToExcelDto {
	 ...
	 * @return 分销总成本和分销利润率都不大于0时，返回null，否则返回计算结果
	 */
	public Double calculateDisPrice(double discountRate){
		BigDecimal bigDecimal = new BigDecimal(discountRate);
		if (disTotalCost != null && disTotalCost > 0 && disProfitRate != null && disProfitRate > 0) {
			// 分销商的分销价=总成本 / (1- 利润率*折扣)
			BigDecimal result = new BigDecimal(disTotalCost)
				.divide(new BigDecimal(1)
				.subtract(new BigDecimal(disProfitRate).multiply(bigDecimal))
				,BigDecimal.ROUND_HALF_UP);
			if (warehouseId == 2024) {
				result = result.subtract(new BigDecimal(disFreight));
			}
			Double disPrice = result.setScale(2, BigDecimal.ROUND_HALF_UP).doubleValue();
			// 特价商品？
			if (isSpecial != null && isSpecial) {
				disPrice = specialSale;
			}
			return disPrice;
		}
		return null;
