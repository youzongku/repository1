...
!function($){
    ...
    var _hintTpl = '<div class="ui-loading-block show">'+
        '<div class="ui-loading-cnt ui-hint-cnt ui-center"><%=content%></div></div>';
    ...
    // 构造函数
    var Hint   = function (el,option,isFromTpl) {
        var self=this;
        this.element=$(el);
        this._isFromTpl=isFromTpl;
        this.option=$.extend(defaults,option);
    ...
    Hint.prototype={
        show:function(){
            var e=$.Event('hint:show');
            this.element.trigger(e);
            this.element.show();

        },
        hide :function () {
            var e=$.Event('hint:hide');
            this.element.trigger(e);
            this.element.remove();
        }
    }
    function Plugin(option) {
        ...
        return $.adaptObject(this, defaults, option,_hintTpl,Hint,"hint");
    }
    $.fn.hint=$.hint= Plugin;
}(window.Zepto)
